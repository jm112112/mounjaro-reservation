...
function getAdminData() {
  return JSON.parse(localStorage.getItem("adminData")) || {
    "2.5mg": { distributed: 0, max: 0 },
    "5.0mg": { distributed: 0, max: 0 },
    "7.5mg": { distributed: 0, max: 0 },
    "10.0mg": { distributed: 0, max: 0 }
  };
}
...
function updateDisplay() {
  ...
  const displayText = Object.entries(data)
    .map(([dose, obj]) => `${dose} → 최신 번호: ${obj.max}, 문자 완료: ${obj.distributed}`)
    .join("\n");
